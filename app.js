        ...
        container.innerHTML = `
            <div class="card" style="border: 1px solid #ffa500; background: rgba(255,165,0,0.05);">
                <div class="card-body" style="text-align: center; padding: 40px;">
                    ...
                    <h3 style="color: #fff; margin: 0 0 10px;">Results Pending Verification</h3>
                    <p style="color: #aaa; font-size: 13px; line-height: 1.5; margin-bottom: 20px;">
                        The battle has ended, but HQ is currently verifying:<br>
...
// 1. OVERRIDE: Render Summary Page (With Lock Logic)
async function renderSummary() {
    console.log("📊 Rendering Summary (Locked Mode)...");
    const content = document.getElementById('summary-content');
    if (!content) return;

    // Check if the week is actually over
    if (!isWeekCompleted(STATE.week)) {
        content.innerHTML = `
            <div class="card" style="text-align: center; padding: 40px; border: 1px solid #7b2cbf;">
                <div style="font-size: 40px; margin-bottom: 20px;">⏳</div>
                <h3 style="color: #fff; margin-bottom: 10px;">Battle in Progress</h3>
                <p style="color: #888;">Week ${STATE.week} is still active.</p>
                <div id="bts-countdown-summary"></div>
            </div>`;
        if (typeof renderBTSCountdown === 'function') renderBTSCountdown(); 
        return;
    }

    // --- LOCK LOGIC START ---
    // We check if resultsReleased is explicitely true. 
    // Note: Ensure your backend 'getDashboardData' or 'getWeeklySummary' sends this flag.
    const isReleased = STATE.data?.teamInfo?.resultsReleased === true || STATE.data?.resultsReleased === true;
    ...
    try {
        ...
        // Use existing logic for winners
        const winner = getWeekWinner(teams);
        const sortedTeams = Object.entries(teams).sort(([,a], [,b]) => (b.teamXP||0) - (a.teamXP||0));
        const myTeam = STATE.data?.profile?.team;
            ...
            winnerHTML = `
                <div class="winner-announcement" style="--team-color: ${teamColor(winner)};">
                    ...
                    <div class="winner-content">
                        ...
                        <p class="winner-subtitle">WEEK CHAMPIONS!</p>
                        <div class="winner-xp">
                            <span class="xp-value">${fmt(winnerInfo.teamXP)}</span>
                            <span class="xp-label">Total XP</span>
                        </div>
                    </div>
                    ${winner === myTeam ? `<div class="winner-you-badge">🎉 YOUR TEAM WON!</div>` : ''}
        ...
        let standingsHTML = sortedTeams.map(([name, data], index) => {
            const isQualified = data.trackGoalPassed && data.albumGoalPassed && data.album2xPassed && 
                                data.attendanceConfirmed && data.policeConfirmed;
            
            let statusBadge = isQualified 
                ? '<span style="color:#00ff88">✅ Qualified</span>' 
                : '<span style="color:#ff6b6b">❌ Disqualified</span>';

            return `
                <div class="rank-item" style="border-left: 3px solid ${teamColor(name)};">
                    <div class="rank-num">#${index + 1}</div>
                    <div class="rank-info">
                        <div class="rank-name">${name}</div>
                        <div class="rank-team" style="font-size:10px;">${statusBadge}</div>
                    </div>
                    <div class="rank-xp">${fmt(data.teamXP)} XP</div>
                </div>
            `;
        }).join('');
        ...
        content.innerHTML = `
            <div class="summary-week-header">
                <span class="summary-week-badge">${STATE.week}</span>
                <h2>Final Results</h2>
            </div>
            ${winnerHTML}
            <div class="card standings-card">
                <div class="card-header"><h3>📊 Final Standings</h3></div>
                <div class="card-body standings-list">${standingsHTML}</div>
            </div>
            <div class="summary-actions" style="margin-top: 20px;">
                <button onclick="loadPage('rankings')" class="btn-secondary">👥 View Rankings</button>
                <button onclick="loadPage('home')" class="btn-primary">🏠 Home</button>
            </div>
        `;
    } catch (e) {
        content.innerHTML = '<div class="card"><div class="card-body error-text">Failed to load summary.</div></div>';
    }
}

// 2. OVERRIDE: Admin Verification Tab
async function renderWeekConfirmation() {
    const container = document.getElementById('admin-tab-confirm');
    if (!container) return;

    container.innerHTML = '<div class="loading-text">⏳ Loading verification data...</div>';
    
    try {
        // FIX: Directly fetch summary instead of relying on broken loadAllWeeksData
        const summary = await api('getWeeklySummary', { week: STATE.week });
        ...
        let html = `
            <div style="margin-bottom: 20px; text-align:center;">
                <h3 style="color:#fff; margin:0;">📋 Verification Panel</h3>
                <p style="color:#888; font-size: 12px;">Review teams before releasing results.</p>
            </div>
            <div class="card" style="margin-bottom: 20px; border: 1px solid ${isReleased ? '#00ff88' : '#ffa500'}">
                <div class="card-body" style="padding: 15px; text-align: center;">
                    <p style="color:#fff; margin-bottom: 10px; font-weight:bold;">
                        Status: ${isReleased ? '🟢 LIVE (Visible to Users)' : '🔴 LOCKED (Hidden)'}
                    </p>
                    <button onclick="toggleResultsRelease(!${isReleased})" class="btn-primary" 
                        style="width:100%; background: ${isReleased ? '#ff4444' : '#00ff88'}; color:${isReleased?'#fff':'#000'};">
                        ${isReleased ? '🔒 LOCK RESULTS' : '🚀 RELEASE RESULTS'}
                    </button>
                </div>
            </div>
        `;
        ...
        Object.entries(teams).forEach(([teamName, teamData]) => {
            const attendanceState = teamData.attendanceConfirmed === true ? '✅' : (teamData.attendanceConfirmed === false ? '❌' : '⏳');
            const policeState = teamData.policeConfirmed === true ? '✅' : (teamData.policeConfirmed === false ? '❌' : '⏳');

            html += `
                <div class="card" style="margin-bottom: 15px; border-left: 4px solid ${teamColor(teamName)};">
                    <div class="card-body" style="padding: 15px;">
                        <div style="font-size:16px; font-weight:bold; color:#fff; margin-bottom:10px;">${teamName}</div>
                        
                        <!-- Attendance Row -->
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px; background:rgba(255,255,255,0.05); padding:8px; border-radius:6px;">
                            <span style="color:#ccc; font-size:12px;">📋 Attendance: ${attendanceState}</span>
                            <div>
                                <button onclick="updateTeamStatus('${teamName}', 'attendanceConfirmed', true)" style="font-size:10px; padding:4px 8px; background:#00ff88; border:none; border-radius:4px; cursor:pointer;">Approve</button>
                                <button onclick="updateTeamStatus('${teamName}', 'attendanceConfirmed', false)" style="font-size:10px; padding:4px 8px; background:#ff4444; color:#fff; border:none; border-radius:4px; cursor:pointer;">Reject</button>
                            </div>
                        </div>
                        ...
                        <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(255,255,255,0.05); padding:8px; border-radius:6px;">
                            <span style="color:#ccc; font-size:12px;">👮 Police: ${policeState}</span>
                            <div>
                                <button onclick="updateTeamStatus('${teamName}', 'policeConfirmed', true)" style="font-size:10px; padding:4px 8px; background:#00ff88; border:none; border-radius:4px; cursor:pointer;">Approve</button>
                                <button onclick="updateTeamStatus('${teamName}', 'policeConfirmed', false)" style="font-size:10px; padding:4px 8px; background:#ff4444; color:#fff; border:none; border-radius:4px; cursor:pointer;">Reject</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        console.error(e);
        container.innerHTML = '<p class="error-text">Failed to load verification data.</p>';
    }
}

// 3. FIXED: Update Status via API (Fixed Signature)
async function updateTeamStatus(teamName, field, status) {
    showToast('Updating...', 'info');
    try {
        await api('updateTeamStatus', {
            agentNo: STATE.agentNo,
            adminSession: localStorage.getItem('adminSession'),
            week: STATE.week,
            team: teamName,
            field: field, 
            value: status.toString() // Send "true" or "false" as string
        });
        showToast(`${teamName} updated!`, 'success');
        renderWeekConfirmation(); 
    } catch (e) {
...
async function toggleResultsRelease(release) {
    if(release && !confirm("🚀 RELEASE RESULTS: This will make the winner and XP visible to ALL agents. Proceed?")) return;
    
    showToast('Processing...', 'info');
    try {
        await api('toggleResultsRelease', {
            agentNo: STATE.agentNo,
            adminSession: localStorage.getItem('adminSession'),
            week: STATE.week,
            released: release.toString() // Send "true" or "false"
        });
        
        // Force update local state
        if (!STATE.data) STATE.data = {};
        STATE.data.resultsReleased = release;
        
        showToast(release ? 'Results are now LIVE!' : 'Results are now LOCKED');
        renderWeekConfirmation();
    } catch (e) {
