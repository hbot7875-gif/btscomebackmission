// ===== BTS SPY BATTLE - COMPLETE APP.JS v4.7 (Cool Design + Stable Logic) =====
...
// ==================== INITIALIZATION ====================
function initApp() {
    console.log('🚀 Starting App v4.7 (Cool Designs + Stable Login)...');
    injectDesignCSS(); // The new look
    loading(false);
    setupLoginListeners();
    // Pre-fetch, but don't block UI
...
function injectDesignCSS() {
    if ($('bts-spy-styles')) return;
    const style = document.createElement('style');
    style.id = 'bts-spy-styles';
    style.textContent = `
        /* --- 1. AGENT DRAWER: HOLOGRAPHIC MEDALS --- */
        .cool-badge-grid { 
            display: flex; flex-wrap: wrap; gap: 25px; justify-content: center; padding: 20px 10px; 
        }
        .cool-badge-item { 
            display: flex; flex-direction: column; align-items: center; width: 100px; 
            transition: transform 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); 
        }
        .cool-badge-item:hover { transform: translateY(-8px) scale(1.05); }
        
        .cool-badge-img-wrapper {
            width: 85px; height: 85px; 
            border-radius: 50%; 
            padding: 4px; /* Thickness of ring */
            background: conic-gradient(from 0deg, #ff00cc, #3333ff, #00ffcc, #ff00cc); /* Neon Ring */
        ...
        .cool-badge-img-wrapper.level-badge { background: conic-gradient(from 0deg, #ffd700, #ff8c00, #ffff00, #ffd700); box-shadow: 0 0 20px rgba(255, 215, 0, 0.6); }
        
        .cool-badge-img { 
            width: 100%; height: 100%; object-fit: cover; border-radius: 50%; 
            border: 3px solid #0a0a0f; /* Inner border to separate image from ring */
            background: #000; 
        }
        
        .cool-badge-label { 
            margin-top: 12px; font-size: 12px; font-weight: 700; color: #fff; 
            text-shadow: 0 2px 10px rgba(0,0,0,0.8); text-transform: uppercase; letter-spacing: 1px;
        }
        .cool-badge-desc { font-size: 10px; color: #aaa; margin-top: 3px; font-style: italic; }

        /* --- 2. ADMIN ASSETS: ROUNDED DATA CHIPS --- */
        .admin-asset-grid { 
            display: grid; grid-template-columns: repeat(auto-fill, minmax(80px, 1fr)); gap: 12px; padding: 15px; 
        }
        .admin-asset-item { 
            width: 100%; aspect-ratio: 1; 
            border-radius: 16px; /* Rounded Chip Look */
            overflow: hidden; 
        ...
        .admin-asset-item:hover { 
            transform: scale(1.1); z-index: 100; 
            border-color: #4cc9f0; 
            box-shadow: 0 10px 20px rgba(76, 201, 240, 0.4); 
        }
        .admin-asset-img { width: 100%; height: 100%; object-fit: cover; opacity: 0.8; transition: opacity 0.3s; }
        .admin-asset-item:hover .admin-asset-img { opacity: 1; }
        
        .admin-asset-id { 
            position: absolute; bottom: 0; left: 0; width: 100%; 
            background: rgba(0,0,0,0.85); backdrop-filter: blur(2px);
            font-size: 10px; color: #4cc9f0; text-align: center; 
            padding: 4px 0; font-family: 'Courier New', monospace; letter-spacing: 0.5px;
        }

        /* --- 3. ADMIN PANEL STABILITY (DO NOT REMOVE) --- */
        .admin-panel { 
        ...
        .admin-tab { padding: 8px 16px; border: 1px solid #333; border-radius: 20px; background: transparent; color: #888; cursor: pointer; white-space: nowrap; transition: 0.3s; }
        .admin-tab:hover { color: #fff; border-color: #555; }
        .admin-tab.active { background: #7b2cbf; color: #fff; border-color: #7b2cbf; box-shadow: 0 0 10px rgba(123,44,191,0.4); }
        .admin-tab-content { display: none; }
        .admin-tab-content.active { display: block; animation: fadeIn 0.3s ease; }
        @keyframes fadeIn { from { opacity:0; transform: translateY(5px); } to { opacity:1; transform: translateY(0); } }
        .admin-mission-card { background: #1a1a2e; padding: 15px; border-radius: 8px; margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; border: 1px solid #333; }
    `;
...
// === COOL ADMIN ASSETS RENDER (ROUNDED DATA CHIPS) ===
function renderAdminAssets() {
    const container = $('admin-tab-assets');
    if(!container) return;
    // Maps to .admin-asset-grid CSS
...
// === COOL BADGES FOR DRAWER (NEON RING) ===
async function renderDrawer() {
    ...
    for (let i = 1; i <= level; i++) {
        ...
        // Maps to .cool-badge-img-wrapper CSS
        badgesHtml += `
