...
const NEWS_SYSTEM = {
    ...
    loadCachedNews() {
        try {
            ...
            if (cached) {
                this.newsItems = JSON.parse(cached);
            }
        } catch (e) {
            this.newsItems = [];
        }
    ...
    async checkForUpdates() {
        ...
        try {
            const [summary, rankings] = await Promise.all([
                api('getWeeklySummary', { week: STATE.week }),
                api('getRankings', { week: STATE.week, limit: 50 })
            ]);
            ...
            console.log('News check failed:', e);
        }
    },
    ...
    compareAndGenerateNews(oldData, newData) {
        ...
        for (const team of Object.keys(newTeamRanks)) {
            const oldRank = oldTeamRanks[team];
        ...
        // === CLOSE BATTLE ALERTS ===
        const sortedTeams = Object.entries(newData.teams)
            .sort((a, b) => (b[1].teamXP || 0) - (a[1].teamXP || 0));
        
        if (sortedTeams.length >= 2) {
            const leader = sortedTeams[0];
            const second = sortedTeams[1];
            const xpGap = (leader[1].teamXP || 0) - (second[1].teamXP || 0);
            
            if (xpGap > 0 && xpGap <= 200) {
        ...
        for (const [team, info] of Object.entries(newData.teams)) {
            ...
            if (!oldInfo.trackGoalPassed && info.trackGoalPassed) {
                this.addNews('goal_complete', '🎵', `${team} Track Goals Done!`, 
                    `${team} completed all track goals!`);
            }
            
            if (!oldInfo.albumGoalPassed && info.albumGoalPassed) {
                this.addNews('goal_complete', '💿', `${team} Album Goals Done!`, 
                    `${team} completed all album goals!`);
            }
            
            if (!oldInfo.album2xPassed && info.album2xPassed) {
                this.addNews('goal_complete', '✨', `${team} 2X Complete!`, 
                    `All ${team} members completed the 2X challenge!`, 'high');
            }
        }
    },
    ...
    showEditModal() {
        ...
        modal.innerHTML = `
            ...
            <div class="bio-edit-content">
                <div class="bio-edit-header">
                    <h3>✏️ Edit Your Bio</h3>
                    <button class="bio-edit-close" onclick="this.closest('.bio-edit-modal').remove()">×</button>
                </div>
                
                <textarea class="bio-input" id="bio-input" maxlength="${this.maxLength}" 
                    placeholder="Write something about yourself...">${sanitize(currentBio)}</textarea>
                
                <div class="bio-char-count">
                    <span id="bio-char-current">${currentBio.length}</span>/${this.maxLength}
                </div>
        ...
        if (input && counter) {
            ...
            input.addEventListener('input', () => {
                ...
                counter.style.color = input.value.length > this.maxLength - 10 ? '#ff6b6b' : '#666';
            });
        }
    }
};
...
const MVP_SYSTEM = {
    ...
    async calculateDailyMVPs() {
        try {
            ...
            const agents = rankings.rankings || [];
            ...
            console.error('Failed to calculate MVPs:', e);
            return {};
        }
    },
    ...
    getCachedMVPs() {
        try {
            const today = this.getTodayKey();
            const cached = localStorage.getItem('dailyMVPs_' + today);
    ...
    async getMVPs() {
        ...
        if (!mvps) {
            mvps = await this.calculateDailyMVPs();
        }
        return mvps;
    },
...
async function renderNewsFeed() {
    ...
    const newsItems = NEWS_SYSTEM.newsItems;
...
window.NEWS_SYSTEM = NEWS_SYSTEM;
window.BIO_SYSTEM = BIO_SYSTEM;
window.MVP_SYSTEM = MVP_SYSTEM;
window.setBioText = setBioText;
window.saveBioFromModal = saveBioFromModal;
window.renderNewsFeed = renderNewsFeed;

console.log('📰 News Feed, Bio & MVP Systems loaded! 🏆');
