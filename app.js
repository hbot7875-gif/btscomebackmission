...
// ==================== PRE-SAVE ANNOUNCEMENT POPUP ====================

const PRESAVE_STORAGE_VERSION = 3;

async function checkPresaveAnnouncement() {
    // SESSION PROTECTION: Only check once per session
    if (STATE._presaveCheckedThisSession) {
        return;
    }
    ...
    try {
        const data = await api('getActivePresaveAnnouncement');
        
        if (!data.success || !data.presave) {
            STATE._presaveCheckedThisSession = true;
            return;
        }
        
        const presave = data.presave;
        const presaveId = presave.id;
        ...
        if (hasRespondedToPresave(presaveId)) {
            console.log('✅ User already responded to presave - NOT showing popup');
            STATE._presaveCheckedThisSession = true;
            return;
        }
        
        // REMINDER: If not_now, check reminder time
        const responseData = getPresaveResponse(presaveId);
        if (responseData.response === 'not_now') {
            if (responseData.reminderTime && Date.now() < responseData.reminderTime) {
                console.log('⏰ Presave reminder not due yet');
                STATE._presaveCheckedThisSession = true;
                return;
            }
        }
        ...
        const dismissKey = 'presave_dismissed_session_' + presaveId;
        ...
        STATE._presaveCheckedThisSession = true;
        STATE.currentPresave = presave;
        
        showPresavePopup(presave);
        
    } catch (e) {
        console.log('Error checking presave:', e);
        STATE._presaveCheckedThisSession = true;
    }
}

// ✅ Simple check if user permanently responded
function hasRespondedToPresave(presaveId) {
    if (!STATE.agentNo || !presaveId) return false;
    
    try {
        const keys = [
            `presave_permanent_${STATE.agentNo}_${presaveId}`,
            `presave_v${PRESAVE_STORAGE_VERSION}_${STATE.agentNo}_${presaveId}`,
            `presave_v3_${STATE.agentNo}_${presaveId}`,
            `presave_v2_${STATE.agentNo}_${presaveId}`,
            `presave_response_${STATE.agentNo}_${presaveId}`
        ];
        ...
        for (const key of keys) {
            ...
            if (data) {
                try {
                    ...
                    if (parsed.response === 'yes' || parsed.response === 'already_presaved') {
                        console.log(`✅ Found permanent response in ${key}:`, parsed.response);
                        return true;
                    }
                } catch (e) {
                    if (data === 'yes' || data === 'already_presaved') {
                        return true;
                    }
                }
            }
        ...
        console.error('Error checking presave response:', e);
        return false;
    }
}

function getPresaveResponse(presaveId) {
    if (!STATE.agentNo || !presaveId) return { response: null };
    
    try {
        const permanentKey = `presave_permanent_${STATE.agentNo}_${presaveId}`;
        const permanentData = localStorage.getItem(permanentKey);
        if (permanentData) {
            return JSON.parse(permanentData);
        }
        
        const key = `presave_v${PRESAVE_STORAGE_VERSION}_${STATE.agentNo}_${presaveId}`;
        const data = localStorage.getItem(key);
        if (data) {
        ...
        console.error('Error getting presave response:', e);
        return { response: null };
    }
}

function savePresaveResponse(presaveId, response, extraData = {}) {
    if (!STATE.agentNo || !presaveId) {
        console.error('Cannot save presave response - missing agentNo or presaveId');
        return;
    }
    ...
    const data = {
        response: response,
        timestamp: Date.now(),
        version: PRESAVE_STORAGE_VERSION,
        agentNo: STATE.agentNo,
        ...extraData
    };
    ...
    try {
        ...
        const versionedKey = `presave_v${PRESAVE_STORAGE_VERSION}_${STATE.agentNo}_${presaveId}`;
        ...
        if (verify) {
            ...
            console.error('❌ FAILED to save presave response!');
        }
        
    } catch (e) {
        console.error('Error saving presave response:', e);
        try {
            localStorage.setItem(`presave_simple_${STATE.agentNo}_${presaveId}`, response);
        } catch (e2) {
...
function showPresavePopup(presave) {
    document.querySelectorAll('.presave-announcement-popup').forEach(p => p.remove());
    
    const popup = document.createElement('div');
    popup.className = 'presave-announcement-popup';
    popup.id = 'presave-popup-' + presave.id;
    
    const safeTitle = sanitize(presave.title || 'Pre-Save Mission');
    const safeMessage = sanitize(presave.message || '');
    const safeLinkText = sanitize(presave.linkText || '🎵 Pre-Save Now →');
    
    popup.innerHTML = `
        <style>
            .presave-announcement-popup {
                position: fixed; top: 0; left: 0; right: 0; bottom: 0;
                background: rgba(0, 0, 0, 0.95); z-index: 99999999;
                display: flex; align-items: center; justify-content: center;
                padding: 20px; animation: presaveFadeIn 0.3s ease; backdrop-filter: blur(5px);
            }
            @keyframes presaveFadeIn { from { opacity: 0; } to { opacity: 1; } }
            @keyframes presaveFadeOut { from { opacity: 1; } to { opacity: 0; } }
            @keyframes presaveSlideUp {
                from { transform: translateY(50px) scale(0.95); opacity: 0; }
                to { transform: translateY(0) scale(1); opacity: 1; }
            }
            @keyframes presaveBorderRotate { 0% { filter: hue-rotate(0deg); } 100% { filter: hue-rotate(360deg); } }
            @keyframes presaveGlow {
                0%, 100% { box-shadow: 0 0 30px rgba(30,215,96,0.4); }
                50% { box-shadow: 0 0 50px rgba(30,215,96,0.7); }
            }
            @keyframes presavePulse {
                0%, 100% { transform: scale(1); }
                50% { transform: scale(1.05); }
            }
            .presave-popup-content {
                background: linear-gradient(145deg, #1a1a2e, #0f0f1f);
                border-radius: 20px; border: 3px solid transparent;
                max-width: 480px; width: 100%; overflow: hidden; position: relative;
                animation: presaveSlideUp 0.4s ease, presaveGlow 3s ease-in-out infinite;
            }
            .presave-popup-content::before {
                content: ''; position: absolute; top: -3px; left: -3px; right: -3px; bottom: -3px;
                background: linear-gradient(45deg, #1DB954, #1ed760, #7b2cbf, #00d4ff, #1DB954);
                border-radius: 22px; z-index: -1; animation: presaveBorderRotate 4s linear infinite;
            }
            .presave-close-btn {
            ...
            .presave-close-btn:hover { background: rgba(255,68,68,0.3); color: #ff6b6b; }
            .presave-btn-primary {
                padding: 18px; background: linear-gradient(135deg, #1DB954, #1ed760);
            ...
            .presave-btn-primary:hover {
                transform: scale(1.02);
                box-shadow: 0 8px 25px rgba(30,215,96,0.4);
            }
            .presave-btn-secondary {
            ...
            .presave-btn-secondary:hover { background: rgba(123,44,191,0.3); }
            .presave-btn-tertiary {
                padding: 10px; background: transparent;
                border: 1px solid rgba(255,255,255,0.15); border-radius: 8px;
                color: #666; font-size: 12px; cursor: pointer; width: 100%;
                transition: all 0.2s ease;
            }
            .presave-btn-tertiary:hover { border-color: rgba(255,255,255,0.3); color: #888; }
            .spotify-icon {
        ...
        <div class="presave-popup-content">
            <button class="presave-close-btn" onclick="dismissPresavePopup('${presave.id}')" title="Close">×</button>
            
            <div style="background:linear-gradient(135deg,#1DB954,#1ed760);padding:8px;text-align:center;">
                <span style="color:#000;font-size:11px;font-weight:bold;letter-spacing:2px;">
                    🎵 URGENT: PRE-SAVE NOW ON SPOTIFY 🎵
                </span>
            </div>
            ...
            <div style="background:linear-gradient(135deg,#7b2cbf,#5a1f99);padding:20px 25px;">
                <div style="display:flex;align-items:center;gap:12px;">
                    ...
                    <div>
                        <div style="color:#1DB954;font-size:11px;font-weight:600;">🔥 PRE-SAVE MISSION</div>
            ...
            <div style="padding:25px;">
                <div style="background:rgba(30,215,96,0.15);border:2px solid #1DB954;border-radius:12px;padding:15px;margin-bottom:20px;">
                    <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
                        <span style="font-size:24px;">🚀</span>
                        <div style="color:#1DB954;font-weight:bold;font-size:14px;">ALL AGENTS MUST PARTICIPATE!</div>
                    </div>
                    <div style="color:#fff;font-size:12px;line-height:1.6;">
                        This is a <strong style="color:#ffd700;">team mission</strong>. Every pre-save counts toward 7M goal!
                    </div>
                </div>
                
                <div style="color:#fff;font-size:14px;line-height:1.7;white-space:pre-line;margin-bottom:20px;padding:15px;background:rgba(255,255,255,0.03);border-radius:10px;border-left:4px solid #1DB954;max-height:150px;overflow-y:auto;">
                    ${safeMessage}
                </div>
                
                <div style="background:linear-gradient(135deg,rgba(30,215,96,0.15),rgba(30,215,96,0.05));border:2px solid rgba(30,215,96,0.4);border-radius:12px;padding:18px;margin-bottom:25px;text-align:center;">
                    <div style="font-size:24px;margin-bottom:10px;">💜</div>
                    <div style="color:#1DB954;font-size:15px;font-weight:bold;">Will you help pre-save ARIRANG?</div>
                    <div style="color:#888;font-size:11px;margin-top:5px;">Takes less than 30 seconds!</div>
                </div>
                
                <div style="display:flex;flex-direction:column;gap:12px;">
                    <button id="presave-btn-yes" class="presave-btn-primary" onclick="respondToPresave('yes')">
                        ...
                        <span>Yes! Pre-Save Now!</span>
                        <span>→</span>
                    </button>
                    <button id="presave-btn-done" class="presave-btn-secondary" onclick="respondToPresave('already_presaved')">
                        <span>✅</span><span>I Already Pre-Saved!</span>
                    </button>
                    <button id="presave-btn-later" class="presave-btn-tertiary" onclick="respondToPresave('not_now')">
                        Remind Me Later
                    </button>
                </div>
            </div>
...
function dismissPresavePopup(presaveId) {
    const popup = document.querySelector('.presave-announcement-popup');
    if (popup) {
        popup.style.animation = 'presaveFadeOut 0.3s ease forwards';
        setTimeout(() => {
            popup.remove();
    ...
    if (presaveId) {
        sessionStorage.setItem('presave_dismissed_session_' + presaveId, 'true');
    }
}

// ✅ Main presave response handler
async function respondToPresave(response) {
    const presave = STATE.currentPresave;
    if (!presave || !STATE.agentNo) {
        showToast('❌ Error: Pre-save data not found', 'error');
        return;
    }
    ...
    if (response === 'yes' || response === 'already_presaved') {
        savePresaveResponse(presaveId, response);
        console.log(`✅ Saved ${response} response for presave ${presaveId}`);
    }
    
    const buttonId = {
        'yes': 'presave-btn-yes',
        'already_presaved': 'presave-btn-done',
        'not_now': 'presave-btn-later'
    }[response];
    
    const button = document.getElementById(buttonId);
    
    // Disable all buttons
    document.querySelectorAll('.presave-btn-primary, .presave-btn-secondary, .presave-btn-tertiary')
        .forEach(btn => {
            btn.disabled = true;
    ...
    try {
        ...
        if (response === 'yes') {
            if (presave.link) {
                window.open(presave.link, '_blank', 'noopener,noreferrer');
            }
            if (typeof confetti === 'function') {
                confetti({ 
                    ...
                    spread: 80, 
                    origin: { y: 0.6 },
                    colors: ['#1DB954', '#1ed760', '#ffffff', '#7b2cbf']
                });
            }
            showToast('🎉 Thank you! Spotify opened - Pre-save now!', 'success');
            
        } else if (response === 'already_presaved') {
            if (typeof confetti === 'function') {
                confetti({ 
                    particleCount: 80, 
                    spread: 60, 
                    origin: { y: 0.6 },
                    colors: ['#1DB954', '#7b2cbf', '#ffd700']
                });
            }
            showToast('💜 Thank you for pre-saving ARIRANG!', 'success');
            
        } else if (response === 'not_now') {
            const reminderTime = Date.now() + (4 * 60 * 60 * 1000);
            savePresaveResponse(presaveId, 'not_now', { reminderTime });
            showToast('⏰ We\'ll remind you in 4 hours', 'info');
        }
        
        setTimeout(() => dismissPresavePopup(presaveId), 1500);
        
    } catch (e) {
        console.error('Error recording presave:', e);
        setTimeout(() => dismissPresavePopup(presaveId), 1500);
    }
}
